...
class LQR(StateFeedbackControllerBase):
    def __init__(self, L, path=None, goal_tol=1.0, car_point_fig =None):
    ...
    def u(self, t, w):
        ...
        denom = (1 -d * self.c_s)
        A_c[0, 0] = 0
        A_c[0, 1] =  v* math.cos(theta_e)
        A_c[1, 0] = - v* (self.c_s**2)*math.cos(theta_e) / denom**2
        A_c[1, 1] =  v * (math.sin(theta_e)/denom)
        ...
        d_system = cont2discrete((A_c, B_c, C, D), self.dt, method='euler')
        A = d_system[0]
        B = d_system[1]


        #Tunning variables for smoothness
        self.Q[0,0] =2E1
        self.Q[1,1] =2E2

        self.R = np.diag([1])*2E2

        X = la.solve_discrete_are(d_system[0], d_system[1], self.Q, self.R)
...
lq = LQR(
 L=car.L, path=nom_path, goal_tol=0.25, car_point_fig=fig)
