...
class StateFeedbackController(StateFeedbackControllerBase):
    ...
    def heading_error(self, theta, s):
        ...
        # theta_path = math.atan2(h_p[1],h_p[0])
        # if np.sign(theta_path) != np.sign(theta):
    ...
    def u(self, t, w):
        ...
        A = np.zeros((2, 2))
        A[0, 0] = self.dt
        A[0, 1] = v*math.sin(theta_e)
        A[1, 0] = self.dt
        A[1, 1] = v *  math.cos(theta_e)/(1 -d * self.u0)
        ...
        #delta  = math.atan2(v *delta_fb , self.L) #- delta_ff


        u_ = self.u0 -self.K * d - self.K*theta_e 

        delta_o  = math.atan(u_ * self.L)
