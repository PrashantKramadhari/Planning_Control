...
class NLStateFeedback(StateFeedbackControllerBase):
    def __init__(self, K, L, path=None, goal_tol=1.0, car_point_fig =None):
    ...
    def u(self, t, w):
        ...
        NL_comp = 1 - (theta_e**2)/6 + (theta_e**4)/120 - (theta_e**6)/5040 
        u = self.plan.c(s) - self.K[0,0]*NL_comp*d - self.K[0,1]*theta_e
        delta = np.arctan2(self.L*u,1)  # Steering angle
