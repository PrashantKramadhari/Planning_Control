...
from world import BoxWorld, state_space_from_lattice
...
mission = {
    "start": {
        "id": np.argmin(np.sum((world.st_sp - np.array(start)[:, None]) ** 2, axis=0))
    },
    "goal": {
        "id": np.argmin(np.sum((world.st_sp - np.array(goal)[:, None]) ** 2, axis=0))
    },
}
...
ax.arrow(
    *start[0:2], *start_arrow[0:2], width=arrow_width, edgecolor="b", facecolor="b"
)
...
def next_state(x, world, mp, rev=True, tol=1e-5):
    ...
    if rev:  # With reverse driving
        for i, j in mp.with_end_orientation_index(theta_i):
            ...
            # Create path to next state
            p = state_i[0:2, np.newaxis] + np.row_stack(
...
opt_length = [r["length"] for r in res if r["name"] == "Dijkstra"][
    0
]  # Dijkstra is optimal
...
# YOUR CODE HERE
#state_space_from_lattice
if graph_== 1  :
    out = mp.plan_to_path(world.st_sp[:, mission["start"]["id"]], res[0])
    for i in range(out[0].shape[0]):
        for j in range(out[1].shape[0]):
            if ( i > out[1][j][1]) and ( i < out[1][j][2]) : 
                if out[1][j][0] == -1:            
                    plt.plot(*out[0][i][0:2],'r.')
                else:
                    plt.plot(*out[0][i][0:2],'b.')
    plt.title('Nodes visited during BreadthFirst search')

if graph_== 2  :
    out = mp.plan_to_path(world.st_sp[:, mission["start"]["id"]], res[1])
    for i in range(out[0].shape[0]):
        for j in range(out[1].shape[0]):
            if ( i > out[1][j][1]) and ( i < out[1][j][2]) : 
                if out[1][j][0] == -1:            
                    ...
                    plt.plot(*out[0][i][0:2],'b.')
    plt.title('Nodes visited during Depth First search')


if graph_== 3  :
    out = mp.plan_to_path(world.st_sp[:, mission["start"]["id"]], res[2])
    for i in range(out[0].shape[0]):
        for j in range(out[1].shape[0]):
            if ( i > out[1][j][1]) and ( i < out[1][j][2]) : 
                if out[1][j][0] == -1:            
                    ...
                    plt.plot(*out[0][i][0:2],'b.')
    plt.title('Nodes visited during Dijkstra search')

if graph_== 4  :
    out = mp.plan_to_path(world.st_sp[:, mission["start"]["id"]], res[3])
    for i in range(out[0].shape[0]):
        for j in range(out[1].shape[0]):
            if ( i > out[1][j][1]) and ( i < out[1][j][2]) : 
                if out[1][j][0] == -1:            
                    ...
                    plt.plot(*out[0][i][0:2],'b.')
    plt.title('Nodes visited during Astar search')

if graph_== 5  :
    out = mp.plan_to_path(world.st_sp[:, mission["start"]["id"]], res[4])
    for i in range(out[0].shape[0]):
        for j in range(out[1].shape[0]):
            if ( i > out[1][j][1]) and ( i < out[1][j][2]) : 
                if out[1][j][0] == -1:            
                    ...
                    plt.plot(*out[0][i][0:2],'b.')
    plt.title('Nodes visited during Best First search')
