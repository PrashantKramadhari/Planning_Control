...
class MotionPrimitives:
    ...
    def plot(self, *args, ax=None, **kwargs):
        ...
        for mp in self.mprims:
            for mpi in mp:
                ax.plot(mpi['x'], mpi['y'], *args, **kwargs)
    ...
    def plan_to_path(self, x0, plan):
        ...
        arguments:
            x0 -- Initial state
            plan -- Plan (full output from the planner)
...
def compute_motion_primitives(theta_init, state_0, L, v, u_max, print_level=3):
    ...
    def compute_mprim(state_i, lattice, L, v, u_max, print_level):
        ...
        for state_f in lattice:
            ...
            C = ca.collocation_interpolators(tau)
            if type(C) is tuple:  # Strange hack; casadi behaves differently
                C = C[0]
