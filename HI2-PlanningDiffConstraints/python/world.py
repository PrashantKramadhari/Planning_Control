...
class BoxWorld:
    ...
    def ObstacleFree_2_pt(self, p,q):
        """ Check if there is any obstacle between 2 points 

        Input
          p - numpy array with 2 rows and m columns, where each column represents a point to be checked
          q - numpy array with 2 rows and m columns, where each column represents a point to be checked
        ...
        """
        for ii in range(p.shape[1]):
            if obstacle_check_2(p[0, ii], p[1, ii], q[0, ii], q[1, ii], self.x_obst, self.y_obst):
                return False
        return True
...
def obstacle_check_2(x1, y1, x2, y2 ,x_obst, y_obst):
    """Help function to function obstacle_free, to check collision for a 
        single point x,y"""
    for ii in range(x_obst.shape[0]):
        if (x1 > x_obst[ii, 0] and x1 < x_obst[ii, 1]) and \
           (x2 > x_obst[ii, 0] and x2 < x_obst[ii, 1]) and \
           (y1 > y_obst[ii, 0] and y1 < y_obst[ii, 1]) and \
           (y2 > y_obst[ii, 0] and y2 < y_obst[ii, 1]):
            return True
    return False
